    ...
    return {
        ...
        link: function (scope, element, attrs, ngModel) {
            ...
            if (opts.showAddBtn) {
                ...
                viewSetting = opts.view.angular.extend({
                    addHoverDom: addHoverDom,
                    removeHoverDom: removeHoverDom
                });
            }
            else {
                viewSetting = opts.view
            }
            //Tree 设置
            var setting = {
                callback: {
...
app.directive('cwServercheck', ['sysService', '$modal', "errorModal", 'loading', function (sysService, $modal, errorModal, loading) {
    return {
        ...
        template: '<div><form ng-submit="cwAdusercheck_checkEmployee()">\
            ...
            <button style="margin-top:-4px;" ng-disabled="dialogDisabled" ng-click="cwAdusercheck_checkEmployee();" type="button" class="btn new-button">\
                  检查姓名\
            </button></form></div>',
...
app.directive('cwhtml', ["$compile", function ($compile) {
    return {
        require: '?ngModel',
        restrict: 'EA',
        link: function ($scope, el, attr) {
            if (attr.cwhtml) {
                var uploadInfo = $scope.$eval(attr.cwhtml);

                var is_obj = uploadInfo['type'];
                if(is_obj){
                    if(uploadInfo.type=='list'){
                        var th = '';
                        var th_key  = [];
                        for(var i = 0; i < uploadInfo['keys'].length; i++){
                            var mid_keys = Object.keys(uploadInfo['keys'][i])[0];
                            if(mid_keys!='is_warn'){
                               th = th+'<th>'+mid_keys+'</th>';
                            th_key.push(uploadInfo['keys'][i][mid_keys])
                            }

                        }
                        var td = '';
                        for(var i = 0; i < uploadInfo['value'].length; i++){
                            td = td+'<tr>';
                            for(var key = 0; key < th_key.length; key++){
                                if(typeof uploadInfo['value'][i][th_key[key]]=='object'){
                                    var mid_json = JSON.stringify(uploadInfo['value'][i][th_key[key]]).replace(/\'/g,'\\"');
                                    td = td+"<td ng-init='b="+mid_json+"' cwhtml='b'></td>"
                                }else {
                                    td = td+"<td>"+uploadInfo['value'][i][th_key[key]]+"</td>"
                                }
                            }
                            td = td+'</tr>';
                        }
                        var end_html = '<table><tr>'+th+'</tr>'+td+'</table>';

                        var ele = $compile(end_html)($scope);
                        angular.element(el).html(ele);
                    }else{
                        var th = '';
                        var td = '';
                        for(var i in uploadInfo['value']){
                            th = th+'<th>'+i+'</th>';
                            if(typeof uploadInfo['value'][i]=='object'){
                                var mid_json = JSON.stringify(uploadInfo['value'][i][th_key[key]]).replace(/\'/g,'\\"');
                                td = td+"<td ng-init='a="+mid_json+"' cwhtml='a'></td>";
                            }else {
                                td = td+"<td>"+uploadInfo['value'][i]+"</td>";
                            }
                        }
                        var end_html = '<table><tr>'+th+'</tr><tr>'+td+'</tr></table>';
                        var ele = $compile(end_html)($scope);
                        angular.element(el).html(ele);
                    }
                }else {
                    angular.element(el).html(uploadInfo['value']);
                }
            }
        }
    }
