controllers.controller("moduleListCtrl", function ($scope, confirmModal, moduleService, loading, msgModal, errorModal, $modal) {
    ...
    $scope.gridOption = {
        ...
        columnDefs: [
            ...
            {field: "os_type_name", displayName: "系统类型", width: 150},
            ...
            {field: "description", displayName: "说明"},
            {
                ...
                '<span class="label label-sm label-info label-btn button-radius" ng-click="checkModule(row.entity)">查看</span>' +
                // '<span  ng-if="!row.entity.is_build_in" class="label label-sm label-info label-btn button-radius" ng-click="modifyModule(row.entity)">修改</span>' +
                '<span class="label label-sm label-danger label-btn button-radius" style="margin-left: 5px;" ng-if="!row.entity.is_build_in" ng-click="deleteModule(row)">删除</span>' +
                '</div>'
            }
        ]
    };
