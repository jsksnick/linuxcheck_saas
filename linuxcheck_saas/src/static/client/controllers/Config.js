...
var CWApp = {
    ...
    isIP: function (ip) {
        ...
        if (ipList[0] === 0 || ipList[3] === 255) return false;
        return true;
    },
    //判断是否为
    isNum: function (num) {
    ...
    isMail: function (mail) {
        ...
        var pattern0 = /[\\/\[\]:;|=,+*?<>@"]/i;
        if (pattern0.test(account)) {
            return false;
        }
        if (account === "") {
            return false;
        }
        var dns = mail.split("@")[1];
    ...
    ValidateDate: function (t, date_str) {
        ...
        var select_time_s = new Date(date_str);
        if (select_time_s === "Invalid Date") {
            return "不是合理的时间格式";
        }
    ...
    ValidateDate2: function (t, date_str) {
        var now_time = t('date')(new Date(), 'yyyy/MM/dd HH:mm:ss');
        var select_time_s = new Date(date_str);
        if (select_time_s === "Invalid Date") {
            return "不是合理的时间格式";
